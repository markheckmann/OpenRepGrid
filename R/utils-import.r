...
pointLabel <- function(x, y = NULL, labels = seq(along = x), cex = 1,
  ...
  nudge <- function(offset) {
    doesIntersect <- rect_intersect(
      xy[rectidx1] + offset[rectidx1],
  ...
  canIntersect <- rect_intersect(
    xy[rectidx1], 2 * rectv[rectidx1],
    xy[rectidx2], 2 * rectv[rectidx2]
  ) > 0
  ...
  if (trace) {
    cat("portion covered =", sum(rect_intersect(
      xy, rectv,
      xy, rectv
    )), "\n")
  }
  ...
  SANN <- function() {
    ...
    for (i in 1:50) {
      ...
      for (j in 1:50) {
        newgene <- gene
        newgene[sample(1:n_labels, 1)] <- sample(
          1:8,
          1
        )
...
errbar <- function(
  ...
  if (is.factor(x) || is.character(x)) {
    ...
    plot(NA, NA, xlab = ylab, ylab = "", xlim = ylim, ylim = c(
      1,
      n + 1
    ), axes = FALSE, ...)
    axis(1)
    ...
    axis(2, at = c(seq.int(length.out = n1), w), labels = c(
      x[t1],
      x[t2]
    ), las = 1, adj = 1)
    points(y[t1], seq.int(length.out = n1),
      pch = pch, type = type,
      ...
    )
    segments(yplus[t1], seq.int(length.out = n1), yminus[t1],
      seq.int(length.out = n1),
      lwd = lwd, lty = lty
    )
    if (any(Type == 2)) {
      abline(h = n1 + 1, lty = 2, ...)
      ...
      axis(side = 3, at = at + offset, labels = format(round(
        at,
        6
      )))
    }
  ...
  if (add) {
    ...
    plot(x, y,
      ylim = ylim, xlab = xlab, ylab = ylab, pch = pch,
      type = type, ...
    )
  }
  xcoord <- par()$usr[1:2]
...
normal.ppplot <- function(x, plot = FALSE) {
  ...
  if (plot) {
    ...
    title(
      main = expression("Normal distribution P-P Plot"),
      sub = paste(
        "Normal P-P plot correlation coefficient:",
        round(corrs.pp, 3)
      )
    )
    abline(0, 1)
...
optimal.boxcox <- function(x, lambda = seq(-2, 2, len = 200), plot = FALSE) {
  ...
  if (plot) {
    ...
    text(
      lambda.max, correlations.pp[m.ind] + 0.05,
